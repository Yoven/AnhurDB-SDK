...
Plus an in-process mock-server proof (no live DB needed) that a transient
HTTP 500 surfaces immediately — the transport performs no client-side retry.
"""
...
# ── No-retry proof against an in-process mock server ─────────────────────────

async def test_no_retry_transient_500():
    print("5. HTTP 500 surfaces immediately with no client-side retry (mock server):")
    ...
    async def flaky_records(request):
        state["attempts"] += 1
        return web.json_response({"error": "transient server error"}, status=500)
    ...
    try:
        async with AnhurClient(api_key="k", url="http://127.0.0.1:8771") as client:
            ...
            # A 5xx surfaces immediately — exactly ONE request, no replay.
            raised = False
            try:
                ...
                raised = True
            check("5xx surfaces as AnhurError", raised)
            check("exactly one attempt (no retry)", state["attempts"] == 1,
                  f"attempts={state['attempts']}")

            # A second failing endpoint behaves identically.
            raised = False
            try:
...
async def main():
    ...
    await test_no_retry_transient_500()
    print()
    if check.failed:
        print("RESULT: FAILED")
        sys.exit(1)
