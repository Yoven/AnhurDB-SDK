// Command anhur-claude-memory is the engine behind the AnhurDB Claude Code memory plugin.
//
...
// client contract we ship to users. It compiles to a SINGLE static binary, which means the hook
// has ZERO runtime dependencies (no python, no jq, no curl).
//
// Two subcommands, wired to Claude Code hooks:
//
//	anhur-claude-memory recall    # SessionStart: flush any queued writes, then print the agent's
//	                              # AnhurDB profile so Claude Code injects it as context.
//	anhur-claude-memory persist   # Stop / SessionEnd: ingest the transcript delta since the last
//	                              # run; on failure, queue to disk and retry on the next recall.
//
...
func loadConfig() config {
	stateDir := envOr("ANHUR_STATE_DIR", filepath.Join(homeDir(), ".anhur-claude-memory"))
	return config{
		apiKey:        os.Getenv("ANHUR_API_KEY"),
		url:           envOr("ANHUR_URL", "http://localhost:8000"),
		container:     envOr("ANHUR_CONTAINER", "claude-ltm"),
		stateDir:      stateDir,
...
func main() {
	...
	if len(os.Args) < 2 {
		logLine(cfg, "usage: anhur-claude-memory <recall|persist>")
		return
	}
	if cfg.apiKey == "" {
	...
	default:
		logLine(cfg, "unknown subcommand: "+os.Args[1])
	}
}
