...
func TestLiveE2E(testHandle *testing.T) {
	...
	// Retry to absorb transient HTTP 500 during cluster maintenance.
	...
	// Live clusters may need a short settle window before content is visible.
	var content string
	var readErr error
	...
	// bare-array). Retrying lets a populated read succeed.
	...
	var entErr error
	for attempt := 1; attempt <= 6; attempt++ {
		entResult, entErr = mem.UpsertEntity(ctx, entName, "organization", "Cognitive memory DB", map[string]interface{}{"lang": "go"})
		if entErr == nil {
			break
		}
		time.Sleep(2 * time.Second)
	...
	if entErr == nil && entResult != nil {
		...
		// Retry: link visibility may lag briefly after write.
...
// absorb brief post-write visibility lag.
//
