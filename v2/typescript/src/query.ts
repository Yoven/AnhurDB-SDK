 ...
 *
 * Junior Tip [SDK parity, 2026-06-18]: mirrors the Python `QueryBuilder`
 ...
 *
 * Junior Tip [contract, verified against server/handler/record_query.go
 ...
 *
 * Junior Tip [contract]: `$neq`/`$nin`/`$like` are deliberately absent — the
...
export class QueryBuilder {
   ...
   *
   * Junior Tip [contract]: the server PARSES but IGNORES `select` — the SQL
  ...
  where(field: string, operator: QueryOperator, value: unknown): this {
    ...
    // Junior Tip [merge per-column]: multiple where() calls on the same column
   ...
   *
   * Junior Tip [parity]: this is the TS analogue of Python
